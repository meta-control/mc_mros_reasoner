...
from mros1_reasoner.tomasys import obtainBestFunctionDesign, print_ontology_status
from mros1_reasoner.tomasys import evaluateObjectives, loadKB_from_file, destroy_entity
from mros1_reasoner.tomasys import resetObjStatus, remove_objective_grounding
...
class RosReasoner(object):
    ...
    def __init__(self):
        ...
        # Whether or not to use system modes reconfiguration / Used mainly for testing
        self.use_reconfiguration_srv = self.check_and_read_parameter("~use_reconfigure_srv", True)

        if self.use_reconfiguration_srv:
            # Get Service name
            self.reconfigure_srv_name =  self.check_and_read_parameter('~reconfigure_srv_name', 'rosgraph_manipulator_action_server')

        # Whether or not to use system modes reconfiguration / Used mainly for testing
        self.use_reconfiguration_srv = self.check_and_read_parameter("~use_reconfigure_srv", True)

        #Start interfaces
        rospy.Subscriber('/diagnostics', DiagnosticArray, self.callbackDiagnostics,)
    ...
    def read_ontology_file(self, ontology_file_name):
        ...
        if ontology_file_name is not None:
            ...
            if ontology is not None:
                ...
                rospy.logerr("Failed to load ontology from: " + str(ontology_file_name))
                return None
        else:
    ...
    # NOTE REFACTORING: This KB initialization is completely mixed with ROS interfaces, probably library should not have an initKB method, but utility methods to update the a-box according to incoming information
    ...
    # - If there is no Objective individual, a navigation Objective is create in the KB, with associated NFR(s) that are read from ros parameters
    def initKB(self):
        ...
        if objectives == []:
            rospy.loginfo('Creating default Objective o_navigateA with default NFR(s)')
            ...
            rospy.loginfo('Objective {} found, NFR(s) and initial FG are generated from the OWL file'.format(objectives[0].name))
        ...
        obj_navigate.o_status = "UNGROUNDED"
        ...
        if self.reasoner.onto is not None and self.hasObjective is True:
            for diagnostic_status in msg.status:
                ...
                if diagnostic_status.message == "binding error":
                    ...
                    if up_binding == -1:
                        rospy.logwarn("Unknown Function Grounding: %s", diagnostic_status.name)
                    elif up_binding == 0:
                        rospy.logwarn("Diagnostics message received for %s with level %d, nothing done about it." % (diagnostic_status.name, diagnostic_status.level))
                ...
                # Component error
                    ...
                    if up_cs == -1:
                        rospy.logwarn("CS message refers to a FG not found in the KB, we assume it refers to the current grounded_configuration (1st fg found in the KB)")
                    elif up_cs == 1:
                        rospy.loginfo("\n\nCS Message received!\tTYPE: {0}\tVALUE: {1}".format(diagnostic_status.values[0].key, diagnostic_status.values[0].value))
                    else:
                        rospy.logwarn("Unsupported CS Message received: %s ", str(diagnostic_status.values[0].key))
                
                # QA Status update
                elif diagnostic_status.message == "QA status":
                    rospy.loginfo("QA value received for: {0}\tTYPE: {1}\tVALUE: {2}".format(diagnostic_status.name, diagnostic_status.values[0].key, diagnostic_status.values[0].value))
                    up_qa = self.reasoner.updateQA(diagnostic_status)
                    if up_qa == -1:
                        rospy.logwarn("No FG found in the KB - Discarding QA message")
                    elif up_qa == 1:
                        rospy.loginfo("QA value received!\tTYPE: {0}\tVALUE: {1}".format(diagnostic_status.values[0].key, diagnostic_status.values[0].value))
                    else:
                        rospy.logwarn("Unsupported QA TYPE received: {}".format(diagnostic_status.values[0].key))
                else:
                    rospy.logwarn("Unsupported Message received: %s ", str(diagnostic_status.values[0].key))
        
    # for MVP with QAs - request the FD.name to reconfigure to
    def request_configuration(self, new_configuration):

        rospy.loginfo('New Configuration requested: {}'.format(new_configuration))
        result = None
        ...
        attempts = 0
        while not rosgraph_manipulator_client.wait_for_server(timeout=rospy.Duration(1.0)) and attempts < 5:
            rospy.logwarn("Action server not found, waiting {}s more".format(5-attempts))
            attempts += 1
    ...
    def timer_cb(self, event):
        ...
        if self.reasoner.isInitialized is not True:
            rospy.loginfo('Waiting to initialize Reasoner -  Nothing else will be done')
            return
        if self.hasObjective is not True:
            rospy.loginfo('Waiting to initialize Objective -  Nothing else will be done')
            return
        ...
        if not objectives_internal_error:
            ...
            for obj_in_error in objectives_internal_error:
                rospy.logwarn("Objective {0} in status {1}".format(obj_in_error.name, obj_in_error.o_status))
        ...
        if obj_in_error.o_status in ["UPDATABLE"]:
            rospy.loginfo("  >> UPDATABLE objective - Try to clear Components status")
            for comp_inst in list(self.reasoner.tomasys.ComponentState.instances()):
                if comp_inst.c_status=="RECOVERED":
                    rospy.loginfo("Component {0} Status {1} - Setting to None".format(
        ...
        if obj_in_error.o_status in ["UNGROUNDED"]:
            ...
            if self.grounded_configuration is not None:
                rospy.loginfo("  >>  UNGROUNDED objective - Try to set to initial FD {0}".format(
        ...
        if not new_grounded:
            rospy.logerr("No FD found to solve Objective {} ".format(obj_in_error.name))
            return
        ...
        rospy.loginfo('Exited timer_cb after successful reconfiguration - Obj set to None')